...
def get_registration_mappings(request):
    ...
    # Mappings for Collage and Affilated
    # mappings['collage'] = list(CategotyCollage.objects.values_list("category_name", "course_name"))

    affiliated_data = CollageAffilated.objects.values("affilated_by", "institute_name")
    grouped_data = {}
    for entry in affiliated_data:
        affilated_by = entry["affilated_by"]
        institute_name = entry["institute_name"]
        if affilated_by not in grouped_data:
            grouped_data[affilated_by] = []
        grouped_data[affilated_by].append(institute_name)

    mappings['collage_affilated'] = grouped_data
    ...
    # chetraToPrants mapping
    raw_data = PrantMaster.objects.select_related("chetra_id").values_list("prant_hindi", "chetra_id__chetra_hindi")
    result = {}
    # Iterate over the data to group by categories
    for item, chetra in raw_data:
        if chetra not in result:
            result[chetra] = []
        result[chetra].append(item)
    mappings['chetraToPrants'] = result
...
# # Function to get registration mappings
def get_chetra_prant_mappings(request):
    mappings = {}
    # chetraToPrants mapping
    raw_data = PrantMaster.objects.select_related("chetra_id").values_list("prant_hindi", "chetra_id__chetra_hindi")
    result = {}
