...
def get_registration_mappings(request):
    ...
    # Fetch data using values, which returns a list of dictionaries
    collage_category = list(CategotyCollage.objects.values("category_name", "course_name"))

    # Group the data by category_name
    grouped_data = {}
    for entry in collage_category:
        category_name = entry["category_name"]
        course_name = entry["course_name"]
        if category_name not in grouped_data:
            grouped_data[category_name] = []
        grouped_data[category_name].append(course_name)

    # Assign to mappings
    mappings['category_name'] = grouped_data


    collage_affiliated = CollageAffilated.objects.values("affilated_by", "institute_name")
    grouped_data = {}
    for entry in collage_affiliated:
    ...
    mappings['collageAffiliated'] = grouped_data
